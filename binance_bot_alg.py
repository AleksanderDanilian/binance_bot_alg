...
# Если передать параметр increase=True то округление произойдет к следующему шагу. Работает. По идее можно было бы и через round такого же добиться.
...
def run_main():
    try:
        ...
        conn = sqlite3.connect('binance_alg.db')
        cursor = conn.cursor()
        ...
        """
        cursor.execute(orders_q)  # функция создания таблицы (если запускаем скрипт 1й раз)
        ...
        """
        # запрос SQL - вывод таблицы orders, где нет отмененных ордеров на покупку и завершенных ордеров на покупку и продажу.
        # Нулевой элемент называется - order_id (buy or sell).

        orders_info = {}

        # формируем словарь из БД orders_q для удобной работы.
        # Формируем его заново после каждого прохода по циклу. Для каждого order_id - свой словарь.
        ...
        # формируем словарь из указанных пар, для удобного доступа (словарь в словаре для разных пар).
        ...
        if orders_info:
            log.debug("Получены неисполненные ордера из БД: {orders}".format(
                orders=[(order, orders_info[order]['order_pair']) for order in
                        orders_info]))  # получаем список order_id и соотв. валютных пар.

            # Проверяем каждый неисполненный по базе ордер
            for order in orders_info:
                # Получаем по ордеру последнюю информацию по бирже
                stock_order_data = bot.orderInfo(symbol=orders_info[order]['order_pair'],
                                                 orderId=order)  # symbol - BTCUSDT, orderID - наш номер неиспользованного ордера.
                log.debug(f'{len(stock_order_data)}, -- длина словаря stock_order_data --')
                ...
                if orders_info[order]['order_type'] == 'buy':
                    # Если ордер уже исполнен (получаем значения по последней сделке - скок купили и почем)
                    if order_status == 'FILLED':
                        ...
                        """.format(
                            ...
                            # узнаем, сколько получили битков за наши доллары(BTCUSDT)
                        ))
                        ...
                        # смотрим, какие ограничения есть для создания ордера на продажу (limits = bot.exchangeInfo())
                        for elem in limits['symbols']:
                            if elem['symbol'] == orders_info[order]['order_pair']:
                                CURR_LIMITS = elem  # находим наши ограничения по торговле в нашей валютной паре среди всех других.
                                break
                        else:
                        ...
                        has_amount = orders_info[order]['buy_amount'] * ((
                                                                                     1 - STOCK_FEE) if not USE_BNB_FEES else 1)  # !!! - если исп. bnb fees, то комиссия расч. отдельно. Не влияет на наши суммы тут.
                        ...
                        need_to_earn = orders_info[order]['buy_amount'] * orders_info[order]['buy_price'] * (
                                1 + all_pairs[stock_order_data['symbol']][
                            'profit_markup'])  # скок купили * почем * на маржу из верхней таблички(40 BTC * 100p * 1.002 = 4008p)
                        # Рассчитываем минимальную цену для продажи
                        min_price = (need_to_earn / sell_amount) / (
                            (1 - STOCK_FEE) if not USE_BNB_FEES else 1)  # (4008 / 40) / 1
                        ...
                        need_price = curr_rate  # убираем вариант с минимальной ценой продажи
                        ...
                        if signal == 'sell':
                            ...
                            if 'orderId' in new_order:
                                log.info(f'{signal}, - ЗНАЧЕНИЕ СИГНАЛА--------------------------')
                                log.info(
                                    "Создан ордер на продажу {new_order}, сигнал {signal}".format(new_order=new_order,
                                                                                                  signal=signal))
                                cursor.execute(
                                    """
                                      UPDATE orders
                            ...
                            # Если были ошибки при создании, выводим сообщение
                                ...
                                log.warning(
                                    "Не удалось создать ордер на продажу {new_order}".format(new_order=new_order))
                ...
                if order_status == 'FILLED' and orders_info[order]['order_type'] == 'sell':
                    ...
                    log.debug("____Баланс {balance}".format(
                        balance=["{k}:{bal:0.8f}".format(k=k, bal=balances[k]) for k in balances]))
                ...
                if balances[pair_obj['base']] >= pair_obj['spend_sum']:
                    ...
                    try:
                        ...
                        if signal == 'buy':
                            ...
                            if 'orderId' in new_order:
                                log.info(
                                    "Создан ордер на покупку {new_order}, сигнал {signal}".format(new_order=new_order,
                                                                                                  signal=signal))
                                cursor.execute(
                                    """
                                      INSERT INTO orders(
                            ...
                            log.info(f'{signal} - ЗНАЧЕНИЕ СИГНАЛА. НЕТ СИГНАЛА НА ПОКУПКУ.')
